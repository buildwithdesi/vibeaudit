          ...
          for (const handler of handlers) {
            ...
            findings.push({
              ...
              fix: isDemoRoute
                ...
                : `Add an auth check at the top of the handler (e.g. "const session = await getServerSession(); if (!session) return Response.json({ error: 'Unauthorized' }, { status: 401 })") or wrap it with your auth guard. If this route is intentionally public, add "// vibe-audit-ignore-next-line missing-auth".`,
            });
          }
          return findings;
        }
      ...
      while ((match = regex.exec(file.content)) !== null) {
        ...
        findings.push({
          ...
          fix: isDemoRoute
            ...
            : `Verify the user's session/token at the top of every handler. If the route is intentionally public, add "// vibe-audit-ignore-next-line missing-auth".`,
        });
      }
    }
    return findings;
