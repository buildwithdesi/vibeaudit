 ...
 * @param {boolean} [cliOptions.quiet] - Suppress report output (for multi-repo mode)
 * @param {AsyncIterable} [cliOptions.fileSource] - Custom file source (e.g. GitHub API). If provided, skips local file discovery.
 * @returns {Promise<{ findings: import('./rules/types.js').Finding[], exitCode: number, filesScanned: number, rulesRun: number, durationMs: number }>}
 */
export async function audit(targetDir, cliOptions = {}) {
  ...
  // Report (skip in quiet mode for multi-repo scanning).
  if (!quiet) {
